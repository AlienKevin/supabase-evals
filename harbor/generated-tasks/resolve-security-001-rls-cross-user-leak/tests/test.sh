...
if [ ! -f "$answer" ]; then
  printf 'FAIL resolve-security-001-rls-cross-user-leak: answer.md is missing\n'
  failed=1
else
...
if grep -Eiq 'owner' "$answer"; then
  ...
  printf 'FAIL answer check 2: owner\n'
  failed=1
fi
...
elif [ -f "$workdir/.oracle/resolve-security-001-rls-cross-user-leak.complete" ]; then
