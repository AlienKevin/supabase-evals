...
if [ ! -f "$answer" ]; then
  printf 'FAIL build-functions-005-dual-auth-user-secret: answer.md is missing\n'
  failed=1
else
...
if grep -Eiq 'user-token' "$answer"; then
  printf 'PASS answer check 2\n'
else
  printf 'FAIL answer check 2: user-token\n'
  failed=1
fi
...
if grep -Eiq 'target user' "$answer"; then
  printf 'PASS answer check 4\n'
else
  printf 'FAIL answer check 4: target user\n'
  failed=1
fi
fi
...
elif [ -f "$workdir/.oracle/build-functions-005-dual-auth-user-secret.complete" ]; then
