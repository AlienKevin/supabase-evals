...
if [ ! -f "$answer" ]; then
  printf 'FAIL resolve-dataapi-002-update-zero-rows-affected: answer.md is missing\n'
  failed=1
else
if grep -Eiq 'tasks' "$answer"; then
  printf 'PASS answer check 1\n'
else
  printf 'FAIL answer check 1: tasks\n'
  failed=1
fi
...
if grep -Eiq 'USING' "$answer"; then
  printf 'PASS answer check 3\n'
else
  printf 'FAIL answer check 3: USING\n'
  failed=1
fi
if grep -Eiq 'WITH CHECK' "$answer"; then
  printf 'PASS answer check 4\n'
else
  printf 'FAIL answer check 4: WITH CHECK\n'
  failed=1
fi
fi
...
elif [ -f "$workdir/.oracle/resolve-dataapi-002-update-zero-rows-affected.complete" ]; then
