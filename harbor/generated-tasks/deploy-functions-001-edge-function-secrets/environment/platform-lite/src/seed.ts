...
export async function loadSeedDir(dir: string): Promise<ProjectSeed[]> {
  ...
  for (const entry of entries) {
    ...
    // Generated Harbor tasks can opt into the bundled pgvector extension
    // without changing the source fixture format. This is intentionally
    // environment-scoped so ordinary platform-lite projects stay lightweight.
    const seed: ProjectSeed = {
      ref: entry.name,
      name: entry.name,
      ...(process.env.PGVECTOR === '1' ? { pgvector: true } : {}),
    };
