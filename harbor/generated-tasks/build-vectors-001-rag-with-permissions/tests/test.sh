...
if [ ! -f "$answer" ]; then
  printf 'FAIL build-vectors-001-rag-with-permissions: answer.md is missing\n'
  failed=1
else
if grep -Eiq 'vector' "$answer"; then
  printf 'PASS answer check 1\n'
else
  printf 'FAIL answer check 1: vector\n'
  failed=1
fi
if grep -Eiq 'HNSW' "$answer"; then
  printf 'PASS answer check 2\n'
else
  printf 'FAIL answer check 2: HNSW\n'
  failed=1
fi
if grep -Eiq 'search' "$answer"; then
  printf 'PASS answer check 3\n'
else
  printf 'FAIL answer check 3: search\n'
  failed=1
fi
...
elif [ -f "$workdir/.oracle/build-vectors-001-rag-with-permissions.complete" ]; then
