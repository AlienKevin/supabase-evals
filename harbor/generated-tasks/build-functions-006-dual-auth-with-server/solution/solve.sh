...
printf '%s\n' 'Built and served user-stats with @supabase/server, preserving RLS for mobile callers and limiting the trusted service path.' > "$workdir/answer.md"
...
printf '%s\n' 'oracle-complete:build-functions-006-dual-auth-with-server' > "$workdir/.oracle/build-functions-006-dual-auth-with-server.complete"
