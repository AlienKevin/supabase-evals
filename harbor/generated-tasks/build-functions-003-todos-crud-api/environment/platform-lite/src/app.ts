import { Hono } from 'hono';
import { type App } from '@supabase/lite';
import { createProjectStore, type ProjectStore } from './project-store.js';
import { ProjectInstance } from './project/ProjectInstance.js';
import { loadSeedDir } from './seed.js';
import { createAccountRoutes } from './management-api/account.js';
import { createDatabaseRoutes } from './management-api/database.js';
import { createFunctionsRoutes } from './management-api/functions.js';
import { createSecretsRoutes } from './management-api/secrets.js';
import { createDebuggingRoutes } from './management-api/debugging.js';
import { createDevelopmentRoutes } from './management-api/development.js';
import { createOpenApiRoutes } from './management-api/openapi.js';
import { listen } from './listen.js';
import type { ListenOptions } from './listen.js';
import { startPgWireServer, type PgServerHandle } from './project/pg-wire.js';
import {
  invokeEdgeFunction,
  type EdgeFunctionsInvokeInput,
} from './project/invoke.js';
import type { AppOptions } from './types.js';

export interface ServerHandle extends AsyncDisposable {
  readonly url: string;
  dispose(): Promise<void>;
  [Symbol.asyncDispose](): Promise<void>;
}

export type { PgServerHandle };

export interface PlatformHandle extends AsyncDisposable {
  readonly app: Hono;
  getProject(ref: string): ProjectInstance | undefined;
  refs(): string[];
  /** Start the HTTP gateway (Management API / data-API routes). */
  listen(options?: ListenOptions): Promise<ServerHandle>;
  /**
   * Start the Postgres-wire endpoint (the "pooler") for DB CLI workflows.
   * A single listener serves every project, routing by the connection's
   * tenant (`user = postgres.<ref>`). Returned handle must be closed by the
   * caller, like `listen()`.
   */
  listenPg(options?: ListenOptions): Promise<PgServerHandle>;
  dispose(): Promise<void>;
  [Symbol.asyncDispose](): Promise<void>;
}

async function build(
  options: AppOptions
): Promise<{ app: Hono; store: ProjectStore }> {
  const { accessToken, projects = [], seedDir } = options;

  const store = createProjectStore();

  const seeds = [...projects];
  if (seedDir) {
    const dirSeeds = await loadSeedDir(seedDir);
    seeds.push(...dirSeeds);
  }

  for (const seed of seeds) {
    const ref = seed.ref ?? generateRef();
    const name = seed.name ?? ref;
    const instance = new ProjectInstance(ref, name, 'default-org');
    await instance.init(seed.sql, seed.logs, seed.functions, seed.pgvector);
    store.set(ref, instance);
  }

  const app = new Hono();
  app.get('/health', (c) => c.json({ ok: true, projects: store.size }));

  // `@supabase/lite` owns the data-plane semantics (PostgREST, GoTrue and
  // Storage). The platform wrapper only supplies the project-ref prefix used
  // by the Management API and the source eval's Supabase URL.
  const dataServers = new Map<string, App['server']>();
  const dataServerFor = (project: ProjectInstance) => {
    let server = dataServers.get(project.ref);
    if (!server) {
      server = project.app.server;
      dataServers.set(project.ref, server);
    }
    return server;
  };

  const routeBundles = [
    createAccountRoutes(store),
    createDatabaseRoutes(store),
    createFunctionsRoutes(store),
    createSecretsRoutes(store),
    createDebuggingRoutes(store),
    createDevelopmentRoutes(store),
  ];

  app.route('/', createOpenApiRoutes());

  if (accessToken !== undefined) {
    app.use('*', async (c, next) => {
      // Management routes are protected by the platform token. Supabase data
      // plane requests carry the project's anon/service JWT and are validated
      // by @supabase/lite instead; function invocation follows the same rule.
      if (isDataPlanePath(c.req.path)) {
        await next();
        return;
      }
      const authHeader = c.req.header('Authorization');
      const token = authHeader?.startsWith('Bearer ')
        ? authHeader.slice(7)
        : authHeader;
      if (token !== accessToken) {
        return c.json({ message: 'Unauthorized' }, 401);
      }
      await next();
    });
  }

  for (const routes of routeBundles) {
    app.route('/', routes.app);
  }

  // HTTP Edge Function invocation for source EVAL.ts contexts. The
  // management deployment routes remain unchanged; this is only the runtime
  // path that Supabase clients use after a function is deployed.
  app.all('/v1/projects/:ref/functions/v1/:slug', (c) => invokeFunctionHttp(c, store));
  app.all('/v1/projects/:ref/functions/v1/:slug/*', (c) => invokeFunctionHttp(c, store));

  // Mount the real lite data server below the platform project prefix. This
  // adapter intentionally leaves auth, RLS, PostgREST and storage behavior to
  // the upstream lite implementation rather than reimplementing a grader.
  app.all('/v1/projects/:ref/*', async (c) => {
    const ref = c.req.param('ref');
    const project = store.get(ref);
    if (!project) return c.json({ message: 'Project not found' }, 404);
    const prefix = `/v1/projects/${ref}/`;
    const wildcard = c.req.path.startsWith(prefix)
      ? c.req.path.slice(prefix.length)
      : '';
    const target = new URL(c.req.url);
    target.pathname = `/${wildcard.replace(/^\/+/, '')}`;
    return dataServerFor(project).fetch(new Request(target, c.req.raw));
  });

  return { app, store };
}

function isDataPlanePath(path: string): boolean {
  return /^\/v1\/projects\/[^/]+\/(?:auth|rest|storage|functions)\/v1(?:\/|$)/.test(path);
}

async function invokeFunctionHttp(c: any, store: ProjectStore): Promise<Response> {
  const ref = c.req.param('ref');
  const name = c.req.param('slug');
  const project = store.get(ref);
  if (!project) return c.json({ message: 'Project not found' }, 404);

  const method = c.req.method.toUpperCase();
  const prefix = `/v1/projects/${ref}/functions/v1/${name}`;
  const wildcard = c.req.path.startsWith(prefix)
    ? c.req.path.slice(prefix.length)
    : '';
  // Hono's `req.path` intentionally excludes the query string. Preserve it
  // when forwarding to the Edge Function so handlers can implement the same
  // query-param routes as a hosted Supabase function.
  const search = new URL(c.req.url).search;
  const body = method === 'GET' || method === 'HEAD'
    ? undefined
    : await c.req.text();
  const input: EdgeFunctionsInvokeInput = {
    name,
    method,
    path: wildcard || search ? `/${wildcard}${search}` : undefined,
    headers: Object.fromEntries(c.req.raw.headers.entries()),
    body,
  };
  const result = await invokeEdgeFunction(project, input);
  if (result.type === 'error') return c.json({ error: result.error }, 500);

  const headers = new Headers(result.headers);
  if (result.outboundBearerTokens.length > 0) {
    headers.set(
      'x-supabase-outbound-bearer-tokens',
      result.outboundBearerTokens.join(','),
    );
  }
  return new Response(result.body, { status: result.status, headers });
}

export async function createPlatform(
  options: AppOptions = {}
): Promise<PlatformHandle> {
  const { app, store } = await build(options);

  // Track open pg-wire listeners so dispose() tears them down too. They must be
  // closed before the projects, since each holds a backend bridged to a
  // project's PGlite.
  const pgServers = new Set<PgServerHandle>();

  const dispose = async () => {
    await Promise.all([...pgServers].map((s) => s.close()));
    await Promise.all([...store.values()].map((p) => p.close()));
  };

  return {
    app,
    getProject: (ref) => store.get(ref),
    refs: () => [...store.keys()],
    listen: async (options) => {
      const { port, hostname, close } = await listen(app, options);
      const urlHost = hostname === '0.0.0.0' ? 'localhost' : hostname;
      const url = `http://${urlHost}:${port}`;
      const serverDispose = async () => {
        close();
      };
      return {
        url,
        dispose: serverDispose,
        [Symbol.asyncDispose]: serverDispose,
      };
    },
    listenPg: async (options) => {
      const handle = await startPgWireServer(
        (ref) => store.get(ref),
        () => [...store.keys()],
        { host: options?.hostname, port: options?.port }
      );
      pgServers.add(handle);
      // De-register on explicit close so dispose() doesn't double-close it.
      return {
        ...handle,
        close: async () => {
          pgServers.delete(handle);
          await handle.close();
        },
      };
    },
    dispose,
    [Symbol.asyncDispose]: dispose,
  };
}

function generateRef(): string {
  return crypto.randomUUID().replace(/-/g, '').slice(0, 20);
}
