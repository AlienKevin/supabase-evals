...
if [ ! -f "$answer" ]; then
  printf 'FAIL build-functions-003-todos-crud-api: answer.md is missing\n'
  failed=1
else
if grep -Eiq 'todos-api' "$answer"; then
  printf 'PASS answer check 1\n'
else
  printf 'FAIL answer check 1: todos-api\n'
  failed=1
fi
if grep -Eiq 'GET' "$answer"; then
  printf 'PASS answer check 2\n'
else
  printf 'FAIL answer check 2: GET\n'
  failed=1
fi
if grep -Eiq 'PATCH' "$answer"; then
  printf 'PASS answer check 3\n'
else
  printf 'FAIL answer check 3: PATCH\n'
  failed=1
fi
if grep -Eiq 'DELETE' "$answer"; then
  printf 'PASS answer check 4\n'
else
  printf 'FAIL answer check 4: DELETE\n'
  failed=1
fi
fi
...
elif [ -f "$workdir/.oracle/build-functions-003-todos-crud-api.complete" ]; then
