...
if [ ! -f "$answer" ]; then
  printf 'FAIL build-rls-002-own-todos-client: answer.md is missing\n'
  failed=1
else
...
if grep -Eiq 'SELECT' "$answer"; then
  ...
  printf 'FAIL answer check 2: SELECT\n'
  failed=1
fi
...
if grep -Eiq 'UPDATE' "$answer"; then
  ...
  printf 'FAIL answer check 4: UPDATE\n'
  failed=1
fi
if grep -Eiq 'DELETE' "$answer"; then
  printf 'PASS answer check 5\n'
else
  printf 'FAIL answer check 5: DELETE\n'
  failed=1
fi
fi
...
elif [ -f "$workdir/.oracle/build-rls-002-own-todos-client.complete" ]; then
