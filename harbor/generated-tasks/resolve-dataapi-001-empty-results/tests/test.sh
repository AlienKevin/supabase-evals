...
if [ ! -f "$answer" ]; then
  printf 'FAIL resolve-dataapi-001-empty-results: answer.md is missing\n'
  failed=1
else
if grep -Eiq 'bookmarks' "$answer"; then
  printf 'PASS answer check 1\n'
else
  printf 'FAIL answer check 1: bookmarks\n'
  failed=1
fi
if grep -Eiq 'RLS' "$answer"; then
  printf 'PASS answer check 2\n'
else
  printf 'FAIL answer check 2: RLS\n'
  failed=1
fi
...
elif [ -f "$workdir/.oracle/resolve-dataapi-001-empty-results.complete" ]; then
