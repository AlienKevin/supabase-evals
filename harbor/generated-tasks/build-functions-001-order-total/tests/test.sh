...
if [ ! -f "$answer" ]; then
  printf 'FAIL build-functions-001-order-total: answer.md is missing\n'
  failed=1
else
if grep -Eiq 'order-total' "$answer"; then
  printf 'PASS answer check 1\n'
else
  printf 'FAIL answer check 1: order-total\n'
  failed=1
fi
if grep -Eiq 'POST' "$answer"; then
  printf 'PASS answer check 2\n'
else
  printf 'FAIL answer check 2: POST\n'
  failed=1
fi
if grep -Eiq 'discount' "$answer"; then
  printf 'PASS answer check 3\n'
else
  printf 'FAIL answer check 3: discount\n'
  failed=1
fi
if grep -Eiq 'tax' "$answer"; then
  printf 'PASS answer check 4\n'
else
  printf 'FAIL answer check 4: tax\n'
  failed=1
fi
fi
...
elif [ -f "$workdir/.oracle/build-functions-001-order-total.complete" ]; then
