...
if [ ! -f "$answer" ]; then
  printf 'FAIL build-realtime-001-live-chat-updates: answer.md is missing\n'
  failed=1
else
if grep -Eiq 'messages' "$answer"; then
  printf 'PASS answer check 1\n'
else
  printf 'FAIL answer check 1: messages\n'
  failed=1
fi
...
elif [ -f "$workdir/.oracle/build-realtime-001-live-chat-updates.complete" ]; then
